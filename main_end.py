...
class Main(QtWidgets.QWidget, Ui_Form):
    ...
    def pack_single_fabric(self, fabric_width, fabric_height, items):
        """
        Метод для расчёта раскроя с построением карты размещения изделий.
        Используется только для визуализации (calculate_cutting).
        """
        ...
        for rotation in [False, True]:
            ...
            placements = []
            used = []
            max_cols = int(fabric_width // (temp_items[0]['width'] if not rotation else temp_items[0]['height']))
    ...
    def create_cutting_map(self, fabric_id, width, height, placements, material_name):
        """
        Создаёт карту раскроя с визуальным отображением размещения изделий.
        """
    ...
    def insert_scrap(self, prod_mat_id, scrap_length, scrap_width, scrap_area, db=None):
        """
        Вставляет или обновляет запись обрезков в таблице obrezki.
        Если для данного prod_mat_id уже существует запись с аналогичными размерами (с точностью 0.01),
        то remainder увеличивается, иначе создаётся новая запись.
        """
        query_select = """
            SELECT id, remainder FROM obrezki 
            WHERE prod_mat_id = %s AND ABS(length - %s) < 0.01 AND ABS(width - %s) < 0.01
        """
        query_update = "UPDATE obrezki SET remainder = %s WHERE id = %s"
        query_insert = """
        ...
        if db is None:
            with self.db_manager as db_conn:
                existing = db_conn.execute_query(query_select, (prod_mat_id, scrap_length, scrap_width))
                if existing:
                    new_remainder = existing[0]['remainder'] + scrap_area
                    db_conn.execute_query(query_update, (new_remainder, existing[0]['id']))
                else:
                    db_conn.execute_insert(query_insert, (prod_mat_id, scrap_length, scrap_width, scrap_area))
        else:
            existing = db.execute_query(query_select, (prod_mat_id, scrap_length, scrap_width))
            if existing:
                new_remainder = existing[0]['remainder'] + scrap_area
                db.execute_query(query_update, (new_remainder, existing[0]['id']))
            else:
                db.execute_insert(query_insert, (prod_mat_id, scrap_length, scrap_width, scrap_area))
        logging.debug(f"Updated/Inserted scrap for prod_mat_id {prod_mat_id}: {scrap_length}x{scrap_width}, area {scrap_area}")
    ...
    def calculate_scraps_mathematically(self):
        ...
        Расчёт производится по следующей формуле:
          ...
          calculated_scrap_area = required_fabrics * fabric_area - total_item_area.
        При этом фактическая площадь обрезков ограничивается 30% от площади полотна.
        Если результат превышает порог (например, 5 см²), то происходит обновление/вставка записи обрезков.
        """
        if not self.current_order:
            return
        try:
            ...
            with self.db_manager as db:
                # Выбираем pm.id как prod_mat_id (корректное значение для FK)
                fabric_query = """
                ...
                for material, items in items_by_material.items():
                    ...
                    calculated_scrap_area = required_fabrics * fabric_area - total_item_area
                    # Ограничиваем обрезки не более чем 30% от площади полотна
                    max_scrap_area = 0.30 * fabric_area
                    scrap_area = min(calculated_scrap_area, max_scrap_area)
                    logging.debug(f"Material {material}: total_item_area = {total_item_area}, "
                                  f"required_fabrics = {required_fabrics}, calculated_scrap_area = {calculated_scrap_area}, "
                                  f"capped scrap_area = {scrap_area}")
                    if scrap_area > 5:
                        # Задаём размеры обрезка – для простоты возьмём половину размеров полотна
                        scrap_length = fabric_info['height'] * 0.5
                        scrap_width = fabric_info['width'] * 0.5
                        # Обновляем или вставляем запись (если такая уже есть, то обновляем remainder)
                        self.insert_scrap(fabric_info['prod_mat_id'], scrap_length, scrap_width, scrap_area, db=db)
            ...
            with self.db_manager as db:
                for material_name, shortage in self.shortage_data.items():
                    ...
                    sc_id = db.execute_insert(
                        "INSERT INTO supply_composition (supply_id, material_id, quantity, length, width, cost, unit_quantity, status, location_id, remainder) VALUES (%s, %s, %s, %s, %s, NULL, '6', %s, NULL, %s)",
                        (supply_id, material_id, shortage, length, width, "Ждёт подтверждения", shortage)
                    )
                    oc_res = db.execute_query(
                        "SELECT id FROM order_composition WHERE order_id = %s LIMIT 1",
                        (self.current_order['id'],)
